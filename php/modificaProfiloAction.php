...
if(empty($values['oldPassword'])) array_push($errors, '<a href="#psw">Compila il campo Vecchia <span xml:lang="en">password</span></a>');
if(empty($values['confermaPassword']) && !empty($values['newPassword'])) array_push($errors, '<a href="#conf-psw">Compila il campo Conferma nuova <span xml:lang="en">password</span></a>');
...
if(count($errors)==0){
    ...
    if($values['newPassword']!=$values['confermaPassword'])array_push($errors, '<a href="#conf-psw">Le <span xml:lang="en">password</span> non corrispondono</a>');
    if(!$user->isPasswordCorrect($values['oldPassword']))array_push($errors, '<a href="#psw">La <span xml:lang="en">password</span> corrente non è corretta</a>');
    ...
    if ($values['newPassword']!=null && !preg_match("/^[\w(#$%&=!)]{4,20}$/", $values['newPassword'])) 
        array_push($errors, '<a href="#newpsw">Per il campo Nuova <span xml:lang="en">Password</span> sono ammessi numeri, lettere e i simboli #,$,%,&,=,! da 5 a 20 caratteri</a>');
